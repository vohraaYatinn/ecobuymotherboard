...
export function Header() {
      ...
      if (token && customerData) {
        try {
          ...
          if (storedName) {
            ...
            try {
              const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://192.168.1.36:5000"}/api/customer-auth/profile`, {
                headers: {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://192.168.1.36:5000"}/api/categories`)
        const data = await response.json()
        if (data.success) {
