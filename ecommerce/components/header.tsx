...
export function Header() {
      ...
      if (token && customerData) {
        try {
          ...
          if (storedName) {
            ...
            try {
              const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://api.safartax.com"}/api/customer-auth/profile`, {
                headers: {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://api.safartax.com"}/api/categories`)
        const data = await response.json()
        if (data.success) {
  ...
  const fetchUnreadNotifications = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://api.safartax.com"}/api/notifications/customer?unreadOnly=true&limit=1`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
